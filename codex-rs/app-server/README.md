...
`threadId` is authoritative when it identifies a thread already loaded by this app-server process. In that case, `thread/resume` rejoins the loaded thread, ignores `path`, and returns the current rollout path. This matters after operations such as `thread/revert`, which preserve the stable thread ID while replacing the current rollout. For an unloaded paginated thread, a supplied `path` must still be current; if it is stale, omit `path` and resume by `threadId`.
